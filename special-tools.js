...
function calcStartup() {
  ...
  const contingency = Math.min(get('startup-contingency') / 100, 1);
...
function calcClothingCost() {
  ...
  const defect = Math.min(get('cost-defect') / 100, 0.95);
...
function calcDiscount() {
  ...
  const feeRate = Math.min(get('discount-fee') / 100, 0.99);
  const adRate = Math.min(get('discount-ad') / 100, 0.99);
  const shipping = get('discount-shipping');
  const discountRate = Math.min(get('discount-rate') / 100, 0.99);
...
function calcRoas() {
  ...
  const grossMarginRate = Math.min(get('roas-gross-margin') / 100, 0.99);
  const extraRate = Math.min(get('roas-extra-rate') / 100, 0.99);
...
function calcMarketplace() {
  ...
  const platform = Math.min(get('market-platform') / 100, 0.99);
  const payment = Math.min(get('market-payment') / 100, 0.99);
  const ad = Math.min(get('market-ad') / 100, 0.99);
  const shipping = get('market-shipping');
  const returnRate = Math.min(get('market-return') / 100, 0.95);
  ...
  if (profit < 0) {
    ...
    badge('warning', '채널 의존 위험');
    diagnose('warning', '플랫폼 매출은 만들 수 있지만 이익 여유가 낮습니다. 할인·쿠폰 분담 조건을 추가하면 적자가 될 수 있습니다.');
  } else {
    badge('good', '판매 가능 구조');
    diagnose('good', '현재 입력값 기준으로 채널 판매 후 이익이 남습니다. 실제 정산서와 비교해 수수료율을 주기적으로 업데이트하세요.');
  }
}
